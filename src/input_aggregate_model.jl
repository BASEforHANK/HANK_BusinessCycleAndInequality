...
F[indexes.μw]     = log.(μwPrime) - m_par.ρ_μw * log.(μw) - (1.0 - m_par.ρ_μw) * log.(m_par.μw) # Process for w-markup target
...
F[indexes.LP]  = log.(LP) - (log((q + r - 1.0)/qlag) - log(RB / π))
...
inc              = [τlev .* ((n_par.mesh_y / n_par.H).^tax_prog_scale .* mcw .*w .*
                    N ./ Ht).^(1.0 - τprog)] # capital liquidation Income (q=1 in steady state)
...
F[indexes.w]    = log.(w) - log.(wage(Kserv, Z * mc, N, m_par))  # wages
...
F[indexes.union_Retained]        = log.(union_RetainedPrime) - (union_firm_profits .- unionprofits .+ log.(union_Retained) .* (RB ./ π)) # Accumulation equation, Retained is shifted by 1
...
F[indexes.Retained]        = log.(RetainedPrime) - (firm_profits .- profits .+ log.(Retained) .* (RB ./ π)) # Accumulation equation, Retained is shifted by 1
...
F[indexes.totRetainedY]  = log.(totRetainedY) - ((log(Retained) + log(union_Retained)) / Y)