...
@inbounds @views  begin
    for jj=1:n[3] # Loop over income states
        for kk = 1:n[2] # Loop over capital states
            #mmm = copy(m_star_n[:,kk,jj])
            #itp_m = LinearInterpolation(m_star_n[:,kk,jj], n_par.grid_m; extrapolation_bc=Line())
            #itp_c = LinearInterpolation(m_star_n[:,kk,jj], c_star_n[:,kk,jj]; extrapolation_bc=Line())
            #cc = itp_c(n_par.grid_m)
            #mn = itp_m(n_par.grid_m)
